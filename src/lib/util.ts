...
export function inNamespace(hashValue: string, namespace: [string, number, number]): boolean {
...
function _evalSimpleUrlPart(actual: string, pattern: string, isPath: boolean): boolean {
	try {
...
function _evalURLTarget(url: string, type: UrlTargetType, pattern: string): boolean {
	try {
...
export function getQueryStringOverride(id: string, url: string, numVariations: number) {
	if (!url) {
		return null;
	}
...
export function getAutoExperimentChangeType(exp: AutoExperiment): AutoExperimentChangeType {
	if (
		exp.urlPatterns &&
		exp.variations.some((variation: unknown) => isObj(variation) && 'urlRedirect' in variation)
	) {
		return 'redirect';
	} else if (
		exp.variations.some(
			(variation: unknown) =>
				isObj(variation) && (variation.domMutations || 'js' in variation || 'css' in variation)
		)
	) {
		return 'visual';
	}
