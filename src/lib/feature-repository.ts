...
import type { GrowthBook } from './index.js';
...
export const helpers: Helpers = {
	fetchFeaturesCall: ({ host, clientKey, headers }) => {
		return (polyfills.fetch as typeof globalThis.fetch)(`${host}/api/features/${clientKey}`, {
			headers
		});
	},
	fetchRemoteEvalCall: ({ host, clientKey, payload, headers }) => {
		...
		return (polyfills.fetch as typeof globalThis.fetch)(`${host}/api/eval/${clientKey}`, options);
	},
...
function getCacheKey(instance: GrowthBook): string {
	...
	const ca: Attributes = {};
	cacheKeyAttributes.forEach((key: string | number) => {
...
function promiseTimeout<T>(promise: Promise<T>, timeout?: number): Promise<T | null> {
...
function onNewFeatureData(key: string, cacheKey: string, data: FeatureApiResponse): void {
...
export function startAutoRefresh(instance: GrowthBook, forceSSE: boolean = false): void {
	const key = getKey(instance);
	const cacheKey = getCacheKey(instance);
