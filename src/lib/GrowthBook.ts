import mutate, { type DeclarativeMutation } from 'dom-mutator';
import type {
	ApiHost,
	Attributes,
	AutoExperiment,
...
import type { ConditionInterface } from './types/mongrule.js';
import {
	chooseVariation,
	decrypt,
...
> {
	...
	private _activeAutoExperiments: Map<AutoExperiment, { valueHash: string; undo: () => void }>;
	...
	public async setEncryptedFeatures(
		...
		this.setFeatures(JSON.parse(featuresJSON) as Record<string, FeatureDefinition>);
	}
	...
	public async decryptPayload(
		...
		if (data.encryptedFeatures) {
			data.features = JSON.parse(
				await decrypt(data.encryptedFeatures, decryptionKey || this._ctx.decryptionKey, subtle)
			);
			delete data.encryptedFeatures;
		}
		if (data.encryptedExperiments) {
			data.experiments = JSON.parse(
				await decrypt(data.encryptedExperiments, decryptionKey || this._ctx.decryptionKey, subtle)
			);
	...
	private _updateAllAutoExperiments(forceRerun?: boolean) {
		...
		for (const exp of experiments) {
			...
			if (result?.inExperiment && getAutoExperimentChangeType(exp) === 'redirect') {
				break;
			}
		}
	}
	...
	public getFeatureValue<V extends AppFeatures[K], K extends string & keyof AppFeatures = string>(
		key: K,
		defaultValue: V
	): WidenPrimitives<V> {
	 ...
	 */
	// eslint-disable-next-line
	public feature<V extends AppFeatures[K], K extends string & keyof AppFeatures = string>(
		id: K
	): FeatureResult<V | null> {
		return this.evalFeature(id);
	}

	public evalFeature<V extends AppFeatures[K], K extends string & keyof AppFeatures = string>(
		id: K
	): FeatureResult<V | null> {
		return this._evalFeature(id);
	}

	private _evalFeature<V extends AppFeatures[K], K extends string & keyof AppFeatures = string>(
		id: K,
		...
		if (evalCtx.evaluatedFeatures.has(id)) {
			process.env.NODE_ENV !== 'production' &&
				this.log(`evalFeature: circular dependency detected: ${evalCtx.id} -> ${id}`, {
					from: evalCtx.id,
					to: id
				});
	...
	private _isIncludedInRollout(
		...
		const { hashValue } = this._getHashAttribute(hashAttribute, fallbackAttribute);
		if (!hashValue) {
			return false;
		}
	...
	private _run<T>(experiment: Experiment<T>, featureId: string | null): Result<T> {
		const key = experiment.key;
		const numVariations = experiment.variations.length;

		// 1. If experiment has less than 2 variations, return immediately
		if (numVariations < 2) {
			process.env.NODE_ENV !== 'production' && this.log('Invalid experiment', { id: key });
			return this._getResult(experiment, -1, false, featureId);
		}

		// 2. If the context is disabled, return immediately
		if (this._ctx.enabled === false) {
			process.env.NODE_ENV !== 'production' && this.log('Context disabled', { id: key });
			return this._getResult(experiment, -1, false, featureId);
		}
		...
		if (!foundStickyBucket) {
			...
			if (experiment.groups && !this._hasGroupOverlap(experiment.groups as string[])) {
				process.env.NODE_ENV !== 'production' &&
	...
	private _getTrackKey(experiment: Experiment<unknown>, result: Result<unknown>) {
	...
	private _getResult<T>(
		...
		const meta: Partial<VariationMeta> = experiment.meta ? experiment.meta[variationIndex] : {};
	...
	private _getNavigateFunction(): null | ((url: string) => void | Promise<void>) {
	...
	private _getStickyBucketAttributes(data?: FeatureApiResponse): Record<string, string> {
	...
	private _generateStickyBucketAssignmentDoc(
		...
		const existingAssignments =
			this._ctx.stickyBucketAssignmentDocs && this._ctx.stickyBucketAssignmentDocs[key]
