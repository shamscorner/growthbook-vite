...
import type {
	...
	VarType
} from './types/mongrule.js';
import { paddedVersionString } from './util.js';

const _regexCache: { [key: string]: RegExp } = {};

// The top-level condition evaluation function
export function evalCondition(obj: TestedObj, condition: ConditionInterface): boolean {
...
function evalConditionValue(condition: ConditionValue, value: any) {
	...
	for (const op in condition) {
		if (
			!evalOperatorCondition(op as Operator, value, condition[op as keyof OperatorConditionValue])
		) {
			return false;
		}
	}
...
function evalOperatorCondition(operator: Operator, actual: any, expected: any): boolean {
	switch (operator) {
		case '$veq':
