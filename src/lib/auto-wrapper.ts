...
const COOKIE_NAME = windowContext.uuidCookieName || dataContext.uuidCookieName || 'gbuuid';
...
function getAutoAttributes(dataContext: DOMStringMap, windowContext: WindowContext) {
	...
	if ((windowContext.persistUuidOnLoad || dataContext.persistUuidOnLoad) && useCookies) {
		persistUUID();
	}
...
const gb = new GrowthBook({
	...
	trackingCallback: (e: { key: any }, r: { key: any }) => {
		...
		// GTM - dataLayer
		window.dataLayer && window.dataLayer.push({ event: 'experiment_viewed', ...p });
